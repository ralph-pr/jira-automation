...
myuser = "SVC-jira-reporter"
mypwd = "svc-user-password"
...
#Start of the loop
cursor.execute("select DISTINCT a.id from source_applications ap inner join source_application_cubes ac on ac.source_application_id = ap.id inner join source_cubes c on c.id = ac.source_cube_id inner join accounts a on a.id = c.account_id where ap.application_type = 'jira_local' and a.id > 300 order by a.id") 
row = cursor.fetchone() 
while row: 
